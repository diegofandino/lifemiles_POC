  ...
  return (
	<section className={`flex flex-col justify-center px-5 md:px-20 gap-3 ${messages.length > 0 ? 'py-20' : 'py-0'}`}>
			...
			{messages && messages.map((message, index) => (
				<div className='chat__elements--items flex gap-5 w-full items-center'>
					...
					`} key={index}>
						...
						}} href={message.fileName} className='text-white py-1 px-4 rounded-xl my-6 ' target="_blank" rel="noopener noreferrer">{message.fileName}</a>}
					</div>
					{ message.client && <CharacterIcon classNames={message.client ? 'order-2' : 'order-1'} string={localStorage.getItem('username')} />}
					{ !message.client && (
		...
		<div className='relative'>
			<input
				className={`input-primary shadow-lg max-w-full w-full outline-none pr-20 ${fileName && 'pb-16 pt-6'}`}
				id="chat_input"
				ref={inputRef}
			...
			<div className={`flex gap-1 absolute ${fileName ? 'top-[70%] translate-y-[-70%]' : 'top-[50%] translate-y-[-50%]'}  right-5`}>
				<label for="upload">
		...
		{messages.length === 0 && <div className='flex flex-col md:flex-row gap-3'>
		...
		{messages.length > 0  && PDF_MAXIMUM_SIZE_TEXT && <p className='text-sm text-left -mt-3'>{PDF_MAXIMUM_SIZE_TEXT}</p>}
	</section>
  )
}
